...
#### `-s string`
Target system: `nes`, `chip8`, `dos`. Auto-detects from file extension if not specified.
```bash
retrodisasm -s nes game.bin
retrodisasm -s chip8 program.rom
retrodisasm -s dos program.bin
```
...
retrodisasm -a nasm -o output.asm input.com
```
...
### DOS Disassembly
```bash
retrodisasm -s dos program.com
retrodisasm -s dos -batch "*.com"
retrodisasm -s dos -base 0x0200 program.com
```
...
* **nasm** - For reassembling nasm format output (DOS, verification not supported)
